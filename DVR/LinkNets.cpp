...
// 在这里初始化SOCKET
BOOL CLinkNetS::InitSendingSocket(SOCKET sReq)
{
	if(m_bStopWorking) return FALSE;
	ASSERT(INVALID_SOCKET == m_sReq);
...
{
	if(INVALID_SOCKET == m_sReq) return FALSE;
...
BOOL CLinkNetS::SelfChecking(PAPPLY_GRAM pLinkInfo,BOOL bRegistered)
{
	//// 注意判断的顺序，不能颠倒
...
UINT CLinkNetS::SendReq()
{
	...
	while (!m_bStopWorking) {
		...
		if(WAIT_OBJECT_0 == wait) {
			...
			nSend = sendto(m_sReq,(char*)&m_ReqInfo,sizeof(APPLY_GRAM),0,(SOCKADDR*)&m_NetSAddr,size);
			...
			EnterCriticalSection(&m_csReqInfo);
			EnterCriticalSection(&g_csSocket);
			sendto(m_sReq,(char*)&m_ReqInfo,sizeof(APPLY_GRAM),0,(SOCKADDR*)&m_NetSAddr,size);
			LeaveCriticalSection(&g_csSocket);
			LeaveCriticalSection(&m_csReqInfo);
