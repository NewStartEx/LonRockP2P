...
#define	DVRMAINCHN_NAME	"NttChn.html"
#define DVRMAINCHT_NAME "NttCht.html"
#define	DVRMAINENG_NAME	"NttEng.html"
...
LONG   CHandleHttp::m_lHttpThread = 0;

#define SAFE_DELETE(x) {if(x != NULL) {delete x; x = NULL;}}

void CHandleHttp::IncreaseHttpThreadCount()
{
	InterlockedIncrement(&m_lHttpThread);
}

void CHandleHttp::DecreaseHttpThreadCount()
{
	InterlockedDecrement(&m_lHttpThread);
}
...
//	setsockopt(serverFd,SOL_SOCKET,SO_LINGER,(CHAR*)&on,sizeof(on));
	...
	while(1) 
	{
		...
		if(WSA_WAIT_EVENT_0 == wRe)
		{
			...
			*/
			DWORD dwID;
			IncreaseHttpThreadCount();
			::CreateThread(NULL,0,(LPTHREAD_START_ROUTINE)HandleReqProc,pHandle,0,&dwID);
		}
...
UINT CHandleHttp::HandleReqProc(LPVOID p)
{
	CHandleHttp* pHttp = (CHandleHttp*)p;
	pHttp->HandleReq();
	delete pHttp;
